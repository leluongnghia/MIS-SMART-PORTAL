...
async function run() {
  ...
  for (const mod of PERMISSION_MODULES) {
    limitedAccess[mod.key] = { view: true }; // Just view by default
  }
  ...
  roles.forEach(r => {
    if (r === "ADMIN" || r === "CHAIRMAN" || r === "BGH") {
      ...
      // Give some specific access based on role
      if (r === "HRM") {
  ...
  departments.forEach(d => {
    ...
    // Give some specific access based on department name/code
  ...
  console.log("Done! Seeded permissions for " + roles.length + " roles and " + departments.length + " departments.");
  process.exit(0);
}
