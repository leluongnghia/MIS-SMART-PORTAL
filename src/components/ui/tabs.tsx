...
export function TabsTrigger({ active, children, onClick, className }: { active: boolean; children: ReactNode; onClick: () => void; className?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn('rounded-lg px-3 py-2 text-sm font-bold transition-colors', active ? 'bg-white text-slate-950 shadow-sm dark:bg-slate-800 dark:text-white' : 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white', className)}
    >
      {children}
    </button>
  );
