...
export default function TicketsPage() {
  ...
  const categories = Array.from(new Set(SERVICE_TICKETS.map(t => t.category)));

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'open': return <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 dark:bg-amber-900/50 dark:text-amber-400">Chờ xử lý</Badge>;
