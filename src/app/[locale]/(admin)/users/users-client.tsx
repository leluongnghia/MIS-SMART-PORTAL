...
function SelectField({ label, value, onChange, options, disabled }: { label: string; value: string; onChange: (v: string) => void; options: any[]; disabled?: boolean }) { return <label className="space-y-1 text-xs font-black uppercase text-slate-500">{label}<select disabled={disabled} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-medium normal-case text-slate-800 disabled:opacity-60 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100">{options.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label>; }
